		...
		for {
			...
			case msg, running := <-in:
				if !running {
					...
					close(out) // Immediately close the output channel
					for {
						select {
						case msg, running := <-responseChannel:
							if !running {
								fmt.Println("Response channel closed after test input channel")
								return true
							}
							fmt.Println("Response received for id", msg.Id)
						case <-ctx.Done():
			...
			case msg, running := <-responseChannel:
				if !running {
					...
					for {
						...
						case _, running = <-in:
							if !running {
								fmt.Println("Test input channel closed after response channel")
								return true
							}
						case <-ctx.Done():
							return true
